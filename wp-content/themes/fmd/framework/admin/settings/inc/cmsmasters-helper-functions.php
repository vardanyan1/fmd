...
function alister_bank_get_admin_page() {
	global $pagenow;
...
function alister_bank_default_tab() {
	$current_page = alister_bank_get_admin_page();
...
function alister_bank_get_the_tab() {
	global $pagenow;
	
	$default_tab = alister_bank_default_tab();
...
function alister_bank_settings_page_header() {
	...
	if ($tabs != '') {
		...
		if (class_exists('Cmsmasters_Custom_Fonts') && $page == 'cmsmasters-settings-font') {
			$links[] = '<a class="' . $class . ' cmsmasters_recaptcha_nav_tab' . '" href="edit.php?post_type=cmsmasters_font">' . esc_html__('Custom Fonts', 'fmd') . '</a>';
		}
