...
function alister_bank_breadcrumbs($sep_text = ' / ') {
	global $post;
	
	
	$homeLink = esc_url(home_url('/'));
	
	$homeText = esc_attr__(pll__('Home'));
	...
	if (is_single()) {
		...
		echo '<span>' . esc_html__('Search results for', 'fmd') . ": '" . esc_html(get_search_query()) . "'</span>";
		...
		if (is_tax('post_format', 'post-format-gallery')) {
			echo '<span>' . esc_html_x('Galleries', 'post format archive title', 'fmd') . '</span>';
		} elseif (is_tax('post_format', 'post-format-image')) {
			echo '<span>' . esc_html_x('Images', 'post format archive title', 'fmd') . '</span>';
		} elseif (is_tax('post_format', 'post-format-video')) {
			echo '<span>' . esc_html_x('Videos', 'post format archive title', 'fmd') . '</span>';
		} elseif (is_tax('post_format', 'post-format-audio')) {
			echo '<span>' . esc_html_x('Audio', 'post format archive title', 'fmd') . '</span>';
		}
		...
		echo '<span>' . esc_html__('No breadcrumbs', 'fmd') . '</span>';
	}
}
