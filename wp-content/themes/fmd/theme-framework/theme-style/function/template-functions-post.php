...
function alister_bank_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function alister_bank_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function alister_bank_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['fmd' . '_blog_post_date']) {
			$out .= '<span class="cmsmasters_post_date">' . 
				'<abbr class="published" title="' . esc_attr(get_the_date()) . '">' . 
					esc_html(get_the_date()) . 
				'</abbr>' . 
...
function alister_bank_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'fmd') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author">' . 
		...
		if ($cmsmasters_option['fmd' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
				'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'fmd') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author">' . 
					'<span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span>' . 
				'</a>' . 
...
function alister_bank_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
...
function alister_bank_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = alister_bank_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['fmd' . '_blog_post_tag']) {
...
function alister_bank_post_exc_cont($length = 20, $show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_post_content entry-content">' . "\n" . 
		wpautop(alister_bank_excerpt($length, false)) . 
	'</div>' . "\n");
	
	
	if (alister_bank_excerpt($length, false) != '') {
		if ($show) {
...
function alister_bank_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['fmd' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function alister_bank_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = alister_bank_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = alister_bank_get_global_options();
			
			if ($cmsmasters_option['fmd' . '_blog_post_comment']) {
				$out = alister_bank_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function alister_bank_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
	$out = '<figure>' . 
