...
function alister_bank_profile_heading($cmsmasters_id, $tag = 'h1', $sub_title = false, $tag_sub = 'h3', $show = true) { 
...
function alister_bank_profile_title_nolink($cmsmasters_id, $tag = 'h1', $sub_title = false, $tag_sub = 'h5', $show = true) { 
...
function alister_bank_get_profile_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['fmd' . '_profile_post_cat']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Categories', 'fmd') . ':' . '</div>' . 
...
function alister_bank_get_profile_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['fmd' . '_profile_post_like']) {
			$out = '<div class="profile_details_item">' . 
				'<div class="profile_details_item_title">' . esc_html__('Likes', 'fmd') . ':' . '</div>' . 
...
function alister_bank_get_profile_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = alister_bank_get_comments('cmsmasters_profile_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = alister_bank_get_global_options();
			
			if ($cmsmasters_option['fmd' . '_profile_post_comment']) {
				$out .= '<div class="profile_details_item">' . 
					'<div class="profile_details_item_title">' . esc_html__('Comments', 'fmd') . ':' . '</div>' . 
					'<div class="profile_details_item_desc details_item_desc_comments">' . 
						alister_bank_get_comments('cmsmasters_profile_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function alister_bank_get_profile_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function alister_bank_profile_social_icons($social_icons, $shortcode_id = false, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
...
function alister_bank_profile_open_social_icons($social_icons, $profile_id = false, $title_box = false, $tag = 'h2', $show = true) {
	if ($social_icons != '') {
		$out = '<div class="profile_social_icons">';
...
function alister_bank_profile_open_style_social_icons() {
	...
	if ($social_icons != '') {
		...
		if ($social_icon_styles != '') {
			wp_add_inline_style('fmd-style', $social_icon_styles);
		}
	}
}

add_action('wp_enqueue_scripts', 'alister_bank_profile_open_style_social_icons');
