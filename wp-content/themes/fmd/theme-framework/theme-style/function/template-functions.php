...
/* Post Type Video */
function alister_bank_post_type_video($cmsmasters_id, $video_type = '', $video_link = '', $video_links = '', $size = 'post-thumbnail') {
	if (!post_password_required()) {
		if ($video_type == 'selfhosted' && !empty($video_links) && sizeof($video_links) > 0) {
			...
			alister_bank_thumb($cmsmasters_id, $size, true, false, true, false, true, true, false);
		}
	}
}
...
/* Post Type Slider */
function alister_bank_post_type_slider($cmsmasters_id, $images, $size = 'post-thumbnail', $slider_data = '') {
	if (!post_password_required()) {
		if (sizeof($images) > 1) {
			echo '<div' . 
			...
			alister_bank_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, $images[0]);
		} elseif (has_post_thumbnail()) {
			alister_bank_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, true, true, true, true, false);
		}
	}
}
...
/* Post Type Gallery */
function alister_bank_post_type_gallery($cmsmasters_id, $images, $columns, $size_col = 'post-thumbnail', $size_full = 'post-thumbnail') {
	if (!post_password_required()) {
		...
		if (sizeof($images) > 0 && $images[0] != '') {
			...
			alister_bank_thumb($cmsmasters_id, $size_full, false, 'img_' . $cmsmasters_id, true, true, false, true, false);
		}
	}
}
...
/* Post Type Image */
function alister_bank_post_type_image($cmsmasters_id, $image_link = '', $size = 'cmsmasters-masonry-thumb') {
	if (!post_password_required()) {
		if ($image_link != '') {
			alister_bank_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, $image_link);
		} elseif (has_post_thumbnail()) {
			alister_bank_thumb($cmsmasters_id, $size, false, 'img_' . $cmsmasters_id, false, false, false, true, false);
		}
	}
}
...
/* Post Type Audio */
function alister_bank_post_type_audio($audio_links) {
...
function alister_bank_get_header_search_form($cmsmasters_option) {
	if (
		...
		$out_form = "<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<div class=\"cmsmasters_header_search_form_field\">
				<input type=\"search\" name=\"s\" placeholder=\"" . esc_attr__(pll__('Enter Keywords')) . "\" value=\"\" />
				<button type=\"submit\">" . esc_html__(pll__('Search')) . "</button>
			</div>
		</form>";
		
		
		$out_form = apply_filters('alister_bank_get_header_search_form_filter', $out_form);
...
/* Theme Error Styles */
function alister_bank_theme_error_styles() {
	$out = "";
	...
	if (is_404()) {
		...
		if (
			$cmsmasters_option['fmd' . '_error_bg_img_enable'] && 
			$cmsmasters_option['fmd' . '_error_bg_image'] != ''
		) {
			$error_bg_image = explode('|', $cmsmasters_option['fmd' . '_error_bg_image']);
			...
			$out .= "
	.error .error_bg {
		background-image : " . (!empty($cmsmasters_option['fmd' . '_error_bg_image']) ? 'url(' . ((is_numeric($error_bg_image[0])) ? $error_bg_image_url[0] : $error_bg_image[1]) . ')' : 'none') . ";
		background-position : " . (!empty($cmsmasters_option['fmd' . '_error_bg_pos']) ? $cmsmasters_option['fmd' . '_error_bg_pos'] : 'top center') . ";
		background-repeat : " . (!empty($cmsmasters_option['fmd' . '_error_bg_rep']) ? $cmsmasters_option['fmd' . '_error_bg_rep'] : 'repeat') . ";
		background-attachment : " . (!empty($cmsmasters_option['fmd' . '_error_bg_att']) ? $cmsmasters_option['fmd' . '_error_bg_att'] : 'scroll') . ";
		background-size : " . (!empty($cmsmasters_option['fmd' . '_error_bg_size']) ? $cmsmasters_option['fmd' . '_error_bg_size'] : 'auto') . ";
	}
";
		}
		...
		$out .= "
	.error .error_bg {
		" . cmsmasters_color_css('background-color', $cmsmasters_option['fmd' . '_error_bg_color']) . "
	}
	
	.error .error_title, 
	.error .error_subtitle {
		" . cmsmasters_color_css('color', $cmsmasters_option['fmd' . '_error_color']) . "
	}
";
	}
...
add_action('wp_enqueue_scripts', 'alister_bank_theme_error_styles');



/* Get Page Heading Function */
function alister_bank_page_heading() {
	if (is_404() || is_home()) {
		...
		if (
			...
			$cmsmasters_header_overlaps = $cmsmasters_option['fmd' . '_header_overlaps'] ? 'true' : 'false';
			
			$cmsmasters_heading_alignment = $cmsmasters_option['fmd' . '_heading_alignment'];
			$cmsmasters_heading_scheme = $cmsmasters_option['fmd' . '_heading_scheme'];
			
			$cmsmasters_breadcrumbs = $cmsmasters_option['fmd' . '_breadcrumbs'] ? 'true' : 'false';
		}
				...
				if ($cmsmasters_heading == 'custom') {
					...
					if (is_search()) {
						...
						if (!empty($wp_query->found_posts)) {
							echo sprintf(esc_html(_n('%1$d search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'fmd')), $wp_query->found_posts, get_search_query());
						} else {
							echo sprintf(esc_html__('0 search results for: %s', 'fmd'), get_search_query());
						}
					} elseif (is_archive()) {
						if (is_author()) {
							if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
								echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'fmd'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
							} else {
								echo sprintf(esc_html__('Author: %s', 'fmd'), '<span class="vcard">' . get_the_author() . '</span>');
							}
						} else {
...
function alister_bank_theme_heading_styles() {
	$cmsmasters_option = alister_bank_get_global_options();
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['fmd' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['fmd' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['fmd' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['fmd' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['fmd' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['fmd' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['fmd' . '_heading_bg_size'];
	}
...
add_action('wp_enqueue_scripts', 'alister_bank_theme_heading_styles');



/* Get Social Icons Styles Function */
function alister_bank_theme_social_icons_styles() {
...
add_action('wp_enqueue_scripts', 'alister_bank_theme_social_icons_styles');



/* Get Posts Thumbnail Function */
function alister_bank_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function alister_bank_thumb_rollover($cmsmasters_id, $type = 'post-thumbnail', $rollover = true, $open_link = true, $group = false, $attachment_images = false, $attachment_video_type = false, $attachment_video_link = false, $attachment_video_links = false, $highImg = false, $show = true, $link_redirect = false, $link_url = false, $link_target = false, $placeholder_icon = 'cmsmasters_theme_icon_image', $puzzle_image = '') {
	...
	if ($rollover) {
		...
		$out .= '<div class="cmsmasters_img_rollover">' . 
			'<a href="' . (($link_redirect == 'true' && $link_url != '') ? esc_url($link_url) : esc_url(get_permalink($cmsmasters_id))) . '"' . (($link_redirect == 'true' && $link_target == 'true') ? ' target="_blank"' : '') . ' title="' . esc_attr($cmsmasters_title) . '" class="cmsmasters_open_post_link button">' . esc_html__('Read more', 'fmd') . '</a>' . 
		'</div>';
	} else {
		$out .= '<div class="cmsmasters_img_rollover">' . 
...
function alister_bank_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
function alister_bank_theme_category_styles() {
	$out = "";
	
	
	if (CMSMASTERS_COLORED_CATEGORIES) {
		$args = alister_bank_colored_categories_args();
...
add_action('wp_enqueue_scripts', 'alister_bank_theme_category_styles');



/* Get Page Container Classes */
function alister_bank_get_page_classes($cmsmasters_option, $classes = false) {
	$browser = new Browser();
	...
	echo 'cmsmasters_' . $cmsmasters_option['fmd' . '_theme_layout'] . ' ';
	...
	if ($cmsmasters_option['fmd' . '_fixed_header']) {
		echo 'fixed_header ';
	}
	...
	if ($cmsmasters_option['fmd' . '_header_styles'] == 'r_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['fmd' . '_header_overlaps'];
	}
...
/* Theme Header Styles */
function alister_bank_theme_header_styles() {
	$cmsmasters_option = alister_bank_get_global_options();
	
	$defaults = alister_bank_settings_general_defaults();
	
	
	$header_top_height = (($cmsmasters_option['fmd' . '_header_top_height'] !== '') ? $cmsmasters_option['fmd' . '_header_top_height'] : $defaults[$tab]['fmd' . '_header_top_height']);
	...
	$header_bot_height = (($cmsmasters_option['fmd' . '_header_bot_height'] !== '') ? $cmsmasters_option['fmd' . '_header_bot_height'] : $defaults[$tab]['fmd' . '_header_bot_height']);
	...
	ul.top_line_nav > li > a {
		line-height : " . ($header_top_height - 3) . "px;
	}
...
";
	...
	if ($cmsmasters_option['fmd' . '_custom_css'] != '') {
		$out .= stripslashes($cmsmasters_option['fmd' . '_custom_css']);
	}
	
	wp_add_inline_style('fmd-style', $out);
}

add_action('wp_enqueue_scripts', 'alister_bank_theme_header_styles');
