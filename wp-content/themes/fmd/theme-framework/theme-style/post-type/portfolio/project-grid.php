...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && alister_bank_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		echo '<div class="cmsmasters_project_images_wrap">';
			...
			alister_bank_thumb_rollover(get_the_ID(), $project_thumb_size, false, $rollover, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);			
		...
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
				...
				$title ? alister_bank_project_heading(get_the_ID(), 'h2', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';
				
				
				if ($categories) {
					echo '<div class="cmsmasters_project_cont_info entry-meta">';
						
						alister_bank_get_project_category(get_the_ID(), 'pj-categs', 'page');
						
					echo '</div>';
				}
				
				
				$excerpt ? alister_bank_project_exc_cont() : '';

				if ($comments || $likes) {
				echo '<footer class="cmsmasters_project_footer entry-meta">';
					
					$comments ? alister_bank_get_project_comments('page') : '';
					
					$likes ? alister_bank_get_project_likes('page') : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				alister_bank_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
