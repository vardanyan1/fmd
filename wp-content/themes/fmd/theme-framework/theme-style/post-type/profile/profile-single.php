...
if (
	$cmsmasters_option['fmd' . '_profile_post_cat'] || 
	$cmsmasters_option['fmd' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			alister_bank_profile_title_nolink(get_the_ID(), 'h1', $cmsmasters_profile_subtitle, 'h5');
		echo '</header>';
	}
		...
		echo '<div class="profile_sidebar">';
			...
			if ($profile_details == 'true') {
				echo '<div class="profile_details entry-meta">' . 
					...
					alister_bank_get_profile_likes('post');
					
					alister_bank_get_profile_comments('post');
					
					alister_bank_get_profile_features('details', $cmsmasters_profile_features, false, 'h3', true);
					
					alister_bank_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			alister_bank_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h3', true);
			
			alister_bank_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h3', true);
			
			alister_bank_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h3', true);
			
			
			alister_bank_profile_open_social_icons($cmsmasters_profile_social, get_the_ID(), esc_html__('Socials', 'fmd'), 'h3');
...
if ($cmsmasters_profile_sharing_box == 'true') {
	alister_bank_sharing_box(esc_html__('Like this profile?', 'fmd'), 'h3');
}


if ($cmsmasters_option['fmd' . '_profile_post_nav_box']) {
	$order_cat = (isset($cmsmasters_option['fmd' . '_profile_post_nav_order_cat']) ? $cmsmasters_option['fmd' . '_profile_post_nav_order_cat'] : false);
