...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_archive_type'); ?>>
	...
	if (!post_password_required() && has_post_thumbnail()) {
		echo '<div class="cmsmasters_archive_item_img_wrap">';
			alister_bank_thumb(get_the_ID(), 'cmsmasters-square-thumb');
		echo '</div>';
	}
	?>
			...
			echo '<footer class="cmsmasters_archive_item_info entry-meta">';
				...
				if (get_post_type() == 'post') {
					...
					'<span class="cmsmasters_archive_item_user_name">' . 
						'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" rel="author" title="' . esc_attr__('Posts by', 'fmd') . ' ' . get_the_author_meta('display_name') . '">' . get_the_author_meta('display_name') . '</a>' . 
					'</span>';
				}
				...
				if ($current_tax != '') {
					echo '<span class="cmsmasters_archive_item_category">' . 
						esc_html__(' In', 'fmd') . ' ' . 
						alister_bank_get_the_category_list(get_the_ID(), $current_tax, ', ') . 
					'</span>';
				}
