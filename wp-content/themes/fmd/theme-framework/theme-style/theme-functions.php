...
function alister_bank_register_theme_js_scripts() {
	...
	wp_localize_script('fmd-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'alister_bank_register_theme_js_scripts');



/* Scripts for Admin */
function alister_bank_theme_admin_scripts() {
	$cmsmasters_option = alister_bank_get_global_options();
	...
	wp_localize_script('cmsmasters-theme-settings-toggle', 'cmsmasters_theme_settings', array( 
		'shortname' => 	'fmd', 
		'header_style' => $cmsmasters_option['fmd' . '_header_styles'] 
	));
}

add_action('admin_enqueue_scripts', 'alister_bank_theme_admin_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'fmd'), 
    'footer' => 	esc_html__('Footer Navigation', 'fmd'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'fmd') 
));
...
function alister_bank_fallback_menu($args) {
	$cmsmasters_option = alister_bank_get_global_options();
	...
	echo '<div class="navigation_wrap">' . "\n" . 
		'<ul id="navigation" class="' . 
			'navigation ' . 
			($cmsmasters_option['fmd' . '_header_styles'] == 'default' ? 'mid_nav' : 'bot_nav') . 
		'">' . "\n";
...
function alister_bank_pagination_prev_text() {
 return 'Previous';
}

add_filter('alister_bank_pagination_prev_text_filter', 'alister_bank_pagination_prev_text');


/* Pagination Next Text */
function alister_bank_pagination_next_text() {
 return 'Next';
}

add_filter('alister_bank_pagination_next_text_filter', 'alister_bank_pagination_next_text');



/* Single Project and Profile Layout */
function alister_bank_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'alister_bank_single_project_layout');



/* Search Form */
function alister_bank_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('enter keywords', 'fmd') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'alister_bank_search_form');
