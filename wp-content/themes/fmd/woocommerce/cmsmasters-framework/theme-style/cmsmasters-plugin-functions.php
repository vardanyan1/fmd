...
function alister_bank_woocommerce_register_styles_scripts() {
	// Styles
	wp_enqueue_style('fmd-woocommerce-style', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-style.css', array(), '1.0.0', 'screen');
	
	wp_enqueue_style('fmd-woocommerce-adaptive', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-adaptive.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('fmd-woocommerce-rtl', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/plugin-rtl.css', array(), '1.0.0', 'screen');
	}
	
	
	// Scripts
	wp_enqueue_script('fmd-woocommerce-script', get_template_directory_uri() . '/woocommerce/cmsmasters-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/js/jquery.plugin-script.js', array('jquery'), '1.0.0', true);
	...
	$cmsmasters_image_sizes = alister_bank_get_image_thumbnail_list();
	
	wp_localize_script('fmd-woocommerce-script', 'cmsmasters_woo_script', array( 
...
add_action('wp_enqueue_scripts', 'alister_bank_woocommerce_register_styles_scripts');


/* Register Default Woocommerce Theme Sidebar */
function alister_bank_woocommerce_the_widgets_init() {
	...
	register_sidebar(
		array(
			'name' => esc_html__('Shop Sidebar', 'fmd'), 
			'id' => 'sidebar_shop', 
			'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on shop page.', 'fmd'), 
...
add_action('widgets_init', 'alister_bank_woocommerce_the_widgets_init');


function alister_bank_woocommerce_demo_store($html, $notice) {
...
add_filter('woocommerce_demo_store', 'alister_bank_woocommerce_demo_store', 10, 2);


function alister_bank_woocommerce_support() {
...
add_action('after_setup_theme', 'alister_bank_woocommerce_support');
